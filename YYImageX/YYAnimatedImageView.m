...
@interface YYAnimatedImageView() {
    @package
    NSImage <YYAnimatedImage> *_curAnimatedImage;
    ...
    NSImage *_curFrame; ///< current frame to display
...
@property (nonatomic, strong) NSImage <YYAnimatedImage> *curImage;
@end
...
- (void)main {
    ...
    for (int i = 0; i < max; i++, idx++) {
        @autoreleasepool {
            ...
            if (miss) {
                NSImage *img = [_curImage animatedImageFrameAtIndex:idx];
...
- (void)resetAnimated {
    dispatch_once(&_onceToken, ^{
        ...
        /* TODO
         ...
         CVDisplayLinkSetOutputCallback(_link, displayLinkOutputCallback, (__bridge void *)self);
         CVDisplayLinkStart(_link);
         */
        
        [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(didEnterBackground:) name:NSApplicationDidHideNotification object:nil];
    });
...
- (void)imageChanged {
    ...
    if ([newVisibleImage isKindOfClass:[NSImage class]] &&
        [newVisibleImage conformsToProtocol:@protocol(YYAnimatedImage)]) {
        newImageFrameCount = ((NSImage<YYAnimatedImage> *) newVisibleImage).animatedImageFrameCount;
        if (newImageFrameCount > 1) {
            hasContentsRect = [((NSImage<YYAnimatedImage> *) newVisibleImage) respondsToSelector:@selector(animatedImageContentsRectAtIndex:)];
        }
    }
    ...
    if (hasContentsRect) {
        CGRect rect = [((NSImage<YYAnimatedImage> *) newVisibleImage) animatedImageContentsRectAtIndex:0];
...
- (void)dealloc {
    [_requestQueue cancelAllOperations];
    [[NSNotificationCenter defaultCenter] removeObserver:self name:NSApplicationDidHideNotification object:nil];
#if TARGET_OS_IOS
    [_link invalidate];
#endif
}
...
- (void)step:(CADisplayLink *)link {
    NSImage <YYAnimatedImage> *image = _curAnimatedImage;
    NSMutableDictionary *buffer = _buffer;
    NSImage *bufferedImage = nil;
...
- (void)displayLayer:(CALayer *)layer {
    if (_curFrame) {
        layer.contents = (__bridge id)[_curFrame cgImageRef];
    }
}

- (void)setContentsRect:(CGRect)rect forImage:(NSImage *)image{
...
- (instancetype)initWithCoder:(NSCoder *)aDecoder {
    ...
    NSImage *image = [aDecoder decodeObjectForKey:@"YYAnimatedImage"];
    NSImage *highlightedImage = [aDecoder decodeObjectForKey:@"YYHighlightedAnimatedImage"];
    if (image) {
        self.image = image;
        [self setImage:image withType:YYAnimatedImageTypeImage];
    }
...
- (void)encodeWithCoder:(NSCoder *)aCoder {
    ...
    multi = (ani && ((NSImage <YYAnimatedImage> *)self.image).animatedImageFrameCount > 1);
    ...
    multi = (ani && ((NSImage <YYAnimatedImage> *)self.highlightedImage).animatedImageFrameCount > 1);
