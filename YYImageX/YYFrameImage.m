...
//    CGFloat scale = _NSStringPathScale(firstPath);
    NSImage *firstCG = [[[NSImage alloc] initWithData:firstData] yy_imageByDecoded];
    CGImageRef cgImage = [firstCG cgImageRef];
    /* TODO self = [self initWithCGImage:cgImage scale:scale orientation:UIImageOrientationUp]; */
    CGSize size = CGSizeMake(CGImageGetWidth(cgImage), CGImageGetHeight(cgImage));
    self = [self initWithCGImage:cgImage size: size];
    if (!self) return nil;
    long frameByte = CGImageGetBytesPerRow(cgImage) * CGImageGetHeight(cgImage);
...
//        CGFloat scale = _NSStringPathScale(path);
        ...
        /* TODO return [[NSImage imageWithData:data scale:scale] yy_imageByDecoded];*/
        return [[[NSImage alloc] initWithData:data] yy_imageByDecoded];
    } else if (_imageDatas) {
        if (index >= _imageDatas.count) return nil;
        NSData *data = _imageDatas[index];
//        CGFloat scale = [NSScreen mainScreen].backingScaleFactor;
        return [[[NSImage alloc] initWithData:data] yy_imageByDecoded];
        /* TODO return [[NSImage imageWithData:datascale:scale] yy_imageByDecoded];*/
    } else {
