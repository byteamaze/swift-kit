...
+ (instancetype)frameWithImage:(NSImage *)image {
...
#if TARGET_OS_IOS
        NSImage *image = [NSImage imageWithCGImage:imageRef scale:_scale orientation:_orientation];
#else
...
- (void)addImage:(NSImage *)image duration:(NSTimeInterval)duration {
    ...
    for (int i = 0; i < count; i++) {
        @autoreleasepool {
            ...
            if ([imageSrc isKindOfClass:[NSImage class]]) {
                NSImage *image = imageSrc;
                CGImageRef cgImage = [image cgImageRef];
                if (cgImage) CGImageDestinationAddImage(destination, cgImage, (CFDictionaryRef)frameProperty);
...
- (CGImageRef)_newCGImageFromIndex:(NSUInteger)index decoded:(BOOL)decoded CF_RETURNS_RETAINED {
    ...
    if ([imageSrc isKindOfClass:[NSImage class]]) {
        ...
        image = [[NSImage alloc] initWithContentsOfURL:(NSURL *)imageSrc];
    } else if ([imageSrc isKindOfClass:[NSData class]]) {
...
- (NSData *)_encodeAPNG {
    ...
    for (int i = 0; i < _images.count; i++) {
        ...
        [pngSizes addObject:[NSValue valueWithSize:size]];
    ...
    CGSize firstFrameSize = [(NSValue *)[pngSizes firstObject] sizeValue];
...
+ (NSData *)encodeImage:(NSImage *)image type:(YYImageType)type quality:(CGFloat)quality {
...
+ (NSData *)encodeImageWithDecoder:(YYImageDecoder *)decoder type:(YYImageType)type quality:(CGFloat)quality {
    ...
    for (int i = 0; i < decoder.frameCount; i++) {
        NSImage *frame = [decoder frameAtIndex:i decodeForDisplay:YES].image;
        [encoder addImageWithData:[frame pngData] duration:[decoder frameDurationAtIndex:i]];
    }
    return encoder.encode;
}
...
@implementation NSImage (YYImageCoder)

- (instancetype)yy_imageByDecoded {
    if (self.yy_isDecodedForDisplay) return self;
    CGImageRef imageRef = [self cgImageRef];
    if (!imageRef) return self;
    CGImageRef newImageRef = YYCGImageCreateDecodedCopy(imageRef, YES);
    if (!newImageRef) return self;
#if TARGET_OS_IOS
    NSImage *newImage = [[self.class alloc] initWithCGImage:newImageRef scale:self.scale orientation:self.imageOrientation];
#else
    CGSize size = CGSizeMake(CGImageGetWidth(newImageRef), CGImageGetHeight(newImageRef));
    NSImage *newImage = [[NSImage alloc] initWithCGImage:newImageRef size: size];
#endif
...
- (BOOL)yy_isDecodedForDisplay {
    if (/* TODO self.images.count > 1  || */ [self isKindOfClass:[YYSpriteSheetImage class]]) return YES;
...
- (NSData *)_yy_dataRepresentationForSystem:(BOOL)forSystem {
    ...
    if (!data) {
        CGImageRef cgImage = [self cgImageRef];
        CGImageRef imageRef = cgImage ? (CGImageRef)CFRetain(cgImage) : nil;
        if (imageRef) {
            CGImageAlphaInfo alphaInfo = CGImageGetAlphaInfo(imageRef) & kCGBitmapAlphaInfoMask;
            BOOL hasAlpha = NO;
            if (alphaInfo == kCGImageAlphaPremultipliedLast ||
            ...
            @autoreleasepool {
                CGSize size = CGSizeMake(CGImageGetWidth(imageRef), CGImageGetHeight(imageRef));
                NSImage *newImage = [[NSImage alloc] initWithCGImage:imageRef size: size];
                if (newImage) {
                    if (hasAlpha) {
                        data = [newImage pngData];
                    } else {
                        data = [newImage jpegData: 0.9];
                    }
                }
            }
            CFRelease(imageRef);
...
-(nullable NSData *) pngData {
    id properties = [NSDictionary dictionary];
    CGImageRef cgImage = [self cgImageRef];
    NSBitmapImageRep *bmpImgRef = [[NSBitmapImageRep alloc] initWithCGImage:cgImage];
    return [bmpImgRef representationUsingType:NSBitmapImageFileTypePNG properties:properties];
}

// 压缩为JPEG
-(nullable NSData *) jpegData:(CGFloat) quality {
    id properties = [NSDictionary dictionaryWithObject:
                     [NSNumber numberWithFloat: quality] forKey:NSImageCompressionFactor];
    CGImageRef cgImage = [self cgImageRef];
    NSBitmapImageRep *bmpImgRef = [[NSBitmapImageRep alloc] initWithCGImage:cgImage];
    return [bmpImgRef representationUsingType:NSBitmapImageFileTypeJPEG properties:properties];
}
